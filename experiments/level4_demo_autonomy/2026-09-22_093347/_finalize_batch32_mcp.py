"""Reconcile L5h cycle 20260922T135351Z from MCP history."""
...
ROWS = [
    dict(
        signal_id="20260922T135351Z_EURCAD_2_S2_1",
        symbol="EURCAD",
        direction="BUY",
        ticket=58573180968,
        entry=1.60985,
    ...
    dict(
        signal_id="20260922T135351Z_EURGBP_1_S1_2",
        symbol="EURGBP",
        direction="SELL",
        ticket=58573181105,
        entry=0.85733,
    ...
    dict(
        signal_id="20260922T135351Z_EURUSD_4_S3_3",
        symbol="EURUSD",
        direction="SELL",
        ticket=58573181247,
        entry=1.14557,
    ...
    dict(
        signal_id="20260922T135351Z_USDCAD_2_S2_4",
        symbol="USDCAD",
        direction="BUY",
        ticket=58573181503,
        entry=1.40538,
    ...
    dict(
        signal_id="20260922T135351Z_USDCHF_2_S1_5",
        symbol="USDCHF",
        direction="BUY",
        ticket=58573181799,
        entry=0.81999,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch32 L5h forced five; soft-avoided L5g; mixed 3 BUY/2 SELL; "
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch32 cycle 20260922T135351Z L5h forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits + " | mix 3 BUY / 2 SELL; avoided L5g and GBPUSD/NZDUSD/AUDUSD SELL stack.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T135351Z", "event": "batch32_complete", "results": results},
    )
    append(LOG, {"event": "batch32_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch32_results.json").write_text(
        json.dumps({"cycle_id": "20260922T135351Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T135351Z" not in md:
        md += (
            "\n## Review 20260922T135900Z — Level 4 batch32 L5h forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 3 BUY / 2 SELL; soft-avoided L5g and did not re-stack "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5h book: **flat**. Foreign Client CADCHF not touched.",
        "",
        "## Batch32 (cycle 20260922T135351Z) comment L5h",
        "Forced five with direction mix (3 BUY / 2 SELL). Soft-avoided L5g. MCP blocked. Python MT5.",
        "Expert timed close ~+5m (16:58:51–53 broker).",
        "",
