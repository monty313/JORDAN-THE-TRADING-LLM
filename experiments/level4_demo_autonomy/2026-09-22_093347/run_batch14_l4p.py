"""Level 4 batch14 L4p. Magic 771249.
...
def pick_five() -> list[dict]:
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks14.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4p" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch14 Level4 L4p; MCP blocked; Python MT5; "
...
def main() -> int:
    ...
    (EXP / "batch14_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch14_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4p" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L4p_left": ours_left})
