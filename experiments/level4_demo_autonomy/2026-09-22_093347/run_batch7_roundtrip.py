"""Level 4 batch7 demo round-trip. Magic 771249. Comment L4g.
open_one sends sl = 0.0 (no broker stop). Do not "fix" this back to
SL = max(80, stops_level+spread+2): that 80-point stop closed EURCNH inside
its signal bar. Pick-file sl_points_planned 80 is stale planning only.
"""
...
PICKS = EXP / "_picks7.json"
MCP_EXITS = EXP / "batch7_mcp_exits.json"  # optional post-hoc fill from parent
...
def finalize(signals: list[dict], fills: list[dict], results: list[dict], cycle_id: str) -> None:
    ...
    for sig in signals:
        ...
        outcome = {
            ...
            "post_outcome_review": (
                "Batch7 Level4 demo; MCP trade blocked; Python MetaTrader5; "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Positions re-read after batch7.",
        "",
        f"## Batch7 (cycle {cycle_id})",
        "MCP trade blocked. Python MetaTrader5. Magic 771249, volume 0.01. "
        "SL=max(80 pts, stops_level+spread+2). Comment L4g.",
        "",
...
def main() -> int:
    ...
    log({"event": "batch_done", "ts_utc": utc_now(), "positions": left, "our_L4g_left": ours_left})
