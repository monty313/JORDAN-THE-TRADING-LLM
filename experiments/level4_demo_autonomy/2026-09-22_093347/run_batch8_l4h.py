"""Level 4 batch8 L4h. Magic 771249. SL=max(80, stops+spread+2). Hold 5m in-process."""
...
def pick_five() -> list[dict]:
    ...
    for r in cur:
        ...
        if sym != r["symbol"] or sym in INDEX_LIKE or sym.startswith("X"):
            continue
        if (sym, d) in FORBID:
            continue
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks8.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4h" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def finalize(signals, fills, results, cycle_id):
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": "Batch8 Level4 L4h; MCP blocked; Python MT5; SL=max(80,stops+spread+2).",
                "locked_reason_unchanged": True,
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch8 cycle {cycle_id}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch8_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch8 (cycle {cycle_id}) comment L4h",
...
def main() -> int:
    ...
    (EXP / "batch8_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch8_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4h" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L4h_left": ours_left})
