"""Reconcile L6a cycle 20260922T154609Z from MCP history — real exits required."""
...
ROWS = [
    dict(
        signal_id="20260922T154609Z_USDCAD_1_S3_1",
        symbol="USDCAD",
        direction="BUY",
        ticket=58575980033,
        entry=1.40735,
    ...
    dict(
        signal_id="20260922T154609Z_EURAUD_2_S2_2",
        symbol="EURAUD",
        direction="SELL",
        ticket=58575980180,
        entry=1.61063,
    ...
    dict(
        signal_id="20260922T154609Z_EURGBP_1_S1_3",
        symbol="EURGBP",
        direction="BUY",
        ticket=58575980378,
        entry=0.85783,
    ...
    dict(
        signal_id="20260922T154609Z_EURJPY_1_S2_4",
        symbol="EURJPY",
        direction="SELL",
        ticket=58575980633,
        entry=180.077,
    ...
    dict(
        signal_id="20260922T154609Z_AUDCHF_0_S0_5",
        symbol="AUDCHF",
        direction="SELL",
        ticket=58575980854,
        entry=0.58337,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch49 L6a forced five; soft-avoided L5z; mixed 2 BUY/3 SELL; "
                    "EURJPY 250-pt stop; hold interrupted then resumed; Expert close +5m; "
                    "foreign Client tickets not touched; MCP history exits (no null)."
                ),
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch49 cycle 20260922T154609Z L6a forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix. H-002 not live filter.",
            "common_sense": bits + " | mix 2 BUY / 3 SELL; avoided L5z; EURJPY 250-pt stop.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T154609Z", "event": "batch49_complete", "results": results},
    )
    append(LOG, {"event": "batch49_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch49_results.json").write_text(
        json.dumps({"cycle_id": "20260922T154609Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T154609Z" not in md:
        md += (
            "\n## Review 20260922T155200Z — Level 4 batch49 L6a forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 2 BUY / 3 SELL; soft-avoided L5z; EURJPY 250-pt stop; "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L6a book: **flat**. Foreign Client tickets not touched.",
        "",
        "## Batch49 (cycle 20260922T154609Z) comment L6a",
        "Forced five with direction mix (2 BUY / 3 SELL). Soft-avoided L5z. MCP blocked. Python MT5.",
        "EURJPY used 250-pt stop. Expert timed close ~+5m (18:51:09–11 broker).",
