"""Reconcile L5k cycle 20260922T141354Z from MCP history."""
...
ROWS = [
    dict(
        signal_id="20260922T141354Z_EURAUD_2_S3_1",
        symbol="EURAUD",
        direction="SELL",
        ticket=58573690733,
        entry=1.60934,
    ...
    dict(
        signal_id="20260922T141354Z_USDCHF_1_S3_2",
        symbol="USDCHF",
        direction="BUY",
        ticket=58573691812,
        entry=0.82045,
    ...
    dict(
        signal_id="20260922T141354Z_AUDNZD_0_S0_3",
        symbol="AUDNZD",
        direction="BUY",
        ticket=58573692311,
        entry=1.24123,
    ...
    dict(
        signal_id="20260922T141354Z_EURCHF_2_S2_4",
        symbol="EURCHF",
        direction="SELL",
        ticket=58573692858,
        entry=0.93949,
    ...
    dict(
        signal_id="20260922T141354Z_USDJPY_1_S3_5",
        symbol="USDJPY",
        direction="BUY",
        ticket=58573693241,
        entry=157.304,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch35 L5k forced five; soft-avoided L5j; mixed 3 BUY/2 SELL; "
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch35 cycle 20260922T141354Z L5k forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits + " | mix 3 BUY / 2 SELL; avoided L5j and GBPUSD/NZDUSD/AUDUSD SELL stack.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T141354Z", "event": "batch35_complete", "results": results},
    )
    append(LOG, {"event": "batch35_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch35_results.json").write_text(
        json.dumps({"cycle_id": "20260922T141354Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T141354Z" not in md:
        md += (
            "\n## Review 20260922T141900Z — Level 4 batch35 L5k forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 3 BUY / 2 SELL; soft-avoided L5j; no GBPUSD/NZDUSD/AUDUSD "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5k book: **flat**. Foreign Client CADCHF not touched.",
        "",
        "## Batch35 (cycle 20260922T141354Z) comment L5k",
        "Forced five with direction mix (3 BUY / 2 SELL). Soft-avoided L5j. MCP blocked. Python MT5.",
        "USDJPY used 250-pt stop. Expert timed close ~+5m (17:18:54–56 broker).",
        "",
