"""Close L4j cycle 20260922T110910Z after hold; score; update handoff. No new opens."""
...
MAGIC = 771249
COMMENT_TAG = "L4j"
HOLD_UNTIL = datetime(2026, 9, 22, 11, 14, 13, tzinfo=timezone.utc)
...
def main() -> int:
    ...
    log({"event": "batch10_close_done", "ts_utc": utc_now(), "left": left, "our_L4j_left": ours_left})
    ...
    for r in results:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch10 L4j; MCP blocked; Python MT5; "
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch10 cycle {CYCLE}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": CYCLE, "event": "batch10_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch10 (cycle {CYCLE}) comment L4j",
