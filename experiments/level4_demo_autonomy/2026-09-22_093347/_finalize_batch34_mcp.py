"""Reconcile L5j cycle 20260922T140702Z from MCP history."""
...
ROWS = [
    dict(
        signal_id="20260922T140702Z_EURUSD_4_S3_1",
        symbol="EURUSD",
        direction="SELL",
        ticket=58573544847,
        entry=1.14532,
    ...
    dict(
        signal_id="20260922T140702Z_NZDUSD_4_S1_2",
        symbol="NZDUSD",
        direction="BUY",
        ticket=58573545292,
        entry=0.57346,
    ...
    dict(
        signal_id="20260922T140702Z_EURGBP_2_S1_3",
        symbol="EURGBP",
        direction="SELL",
        ticket=58573545512,
        entry=0.85719,
    ...
    dict(
        signal_id="20260922T140702Z_USDCAD_1_S3_4",
        symbol="USDCAD",
        direction="BUY",
        ticket=58573545766,
        entry=1.40570,
    ...
    dict(
        signal_id="20260922T140702Z_AUDUSD_1_S1_5",
        symbol="AUDUSD",
        direction="BUY",
        ticket=58573546168,
        entry=0.71163,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch34 L5j forced five; soft-avoided L5i; mixed 3 BUY/2 SELL; "
                    "NZDUSD/AUDUSD as BUY not SELL stack; MCP blocked; Python MT5."
                ),
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch34 cycle 20260922T140702Z L5j forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits + " | mix 3 BUY / 2 SELL; avoided L5i and GBPUSD/NZDUSD/AUDUSD SELL stack.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T140702Z", "event": "batch34_complete", "results": results},
    )
    append(LOG, {"event": "batch34_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch34_results.json").write_text(
        json.dumps({"cycle_id": "20260922T140702Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T140702Z" not in md:
        md += (
            "\n## Review 20260922T141210Z — Level 4 batch34 L5j forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 3 BUY / 2 SELL; soft-avoided L5i; NZDUSD/AUDUSD taken BUY "
            "not the banned SELL stack. MCP trade_* blocked; Python MetaTrader5. "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5j book: **flat**. Foreign Client CADCHF not touched.",
        "",
        "## Batch34 (cycle 20260922T140702Z) comment L5j",
        "Forced five with direction mix (3 BUY / 2 SELL). Soft-avoided L5i. MCP blocked. Python MT5.",
        "Expert timed close ~+5m (17:12:02–05 broker).",
        "",
