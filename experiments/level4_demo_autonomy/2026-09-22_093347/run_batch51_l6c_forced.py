"""Level 4 batch51 L6c FORCED. Magic 771249.
...
# Soft prefer: avoid exact L6b pairs when filling five is still possible
SOFT_AVOID = {
    ("USDCHF", "BUY"),
    ("EURUSD", "SELL"),
    ("USDJPY", "BUY"),
...
def pick_five() -> list[dict]:
    ...
    (EXP / "_picks51.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
        ...
        out.append(
            {
                ...
                "warning_context": ["Level 4 demo autonomy; L6c forced five"],
                ...
                "config_hash": "level4-demo-forced-five-l6c",
            }
        )
    return out
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L6c" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def main() -> int:
    ...
    (EXP / "batch51_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch51_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
            ...
            "mode": "forced_five_l6c",
            "soft_avoid_l6b": [f"{a} {b}" for a, b in SOFT_AVOID],
        }
    )
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L6c" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L6c_left": ours_left})
    ...
    append_jsonl(
        DECISION,
        {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch51_l6c_held_closed", "n": len(results)},
    )
    mt5.shutdown()
