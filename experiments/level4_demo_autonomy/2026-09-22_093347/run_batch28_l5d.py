"""Level 4 batch28 L5d. Magic 771249.
...
def pick_five() -> list[dict]:
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks28.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L5d" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def finalize(signals, fills, results, cycle_id):
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch28 Level4 L5d; stale MENTOR_INBOX 07:43 not a stop token; MCP blocked; Python MT5; "
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch28 cycle {cycle_id}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch28_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch28 (cycle {cycle_id}) comment L5d",
...
def main() -> int:
    ...
    (EXP / "batch28_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch28_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L5d" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L5d_left": ours_left})
