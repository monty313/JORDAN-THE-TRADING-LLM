"""Reconcile L5x cycle 20260922T152640Z from MCP history — real exits required."""
...
ROWS = [
    dict(
        signal_id="20260922T152640Z_EURUSD_1_S2_1",
        symbol="EURUSD",
        direction="SELL",
        ticket=58575572665,
        entry=1.14414,
    ...
    dict(
        signal_id="20260922T152640Z_USDCHF_2_S1_2",
        symbol="USDCHF",
        direction="BUY",
        ticket=58575572910,
        entry=0.82126,
    ...
    dict(
        signal_id="20260922T152640Z_USDJPY_1_S1_3",
        symbol="USDJPY",
        direction="BUY",
        ticket=58575573298,
        entry=157.378,
    ...
    dict(
        signal_id="20260922T152640Z_AUDJPY_2_S4_4",
        symbol="AUDJPY",
        direction="SELL",
        ticket=58575573437,
        entry=111.824,
    ...
    dict(
        signal_id="20260922T152640Z_AUDCHF_0_S0_5",
        symbol="AUDCHF",
        direction="BUY",
        ticket=58575573713,
        entry=0.58357,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch46 L5x forced five; soft-avoided L5w; mixed 3 BUY/2 SELL; "
                    "USDJPY/AUDJPY 250-pt stops; foreign Client USDCAD not touched; "
                    "MCP history exits (no null); MCP blocked; Python MT5."
                ),
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch46 cycle 20260922T152640Z L5x forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits
            + " | mix 3 BUY / 2 SELL; avoided L5w; JPY 250-pt stops; Client USDCAD not touched.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T152640Z", "event": "batch46_complete", "results": results},
    )
    append(LOG, {"event": "batch46_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch46_results.json").write_text(
        json.dumps({"cycle_id": "20260922T152640Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T152640Z" not in md:
        md += (
            "\n## Review 20260922T153200Z — Level 4 batch46 L5x forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 3 BUY / 2 SELL; soft-avoided L5w; USDJPY/AUDJPY 250-pt stops; "
            "foreign Client USDCAD not touched; real MCP history exits. "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5x book: **flat**. Foreign Client USDCAD/CADCHF not touched.",
        "",
        "## Batch46 (cycle 20260922T152640Z) comment L5x",
        "Forced five with direction mix (3 BUY / 2 SELL). Soft-avoided L5w. MCP blocked. Python MT5.",
        "USDJPY/AUDJPY used 250-pt stops. Expert timed close ~+5m (18:31:40–43 broker).",
