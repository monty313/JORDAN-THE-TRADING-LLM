"""Level 4 batch10 L4j. Magic 771249.
SL: majors max(80, stops+spread); exotics (JPY/CNH/SEK/…) max(250, stops+spread).
Hold 5m in-process. One attempt. No retry.
"""
...
EXOTIC_CCY = {
    "JPY", "CNH", "CNY", "SEK", "NOK", "DKK", "MXN", "ZAR", "TRY", "HUF", "PLN",
    "CZK", "SGD", "HKD", "RUB", "INR", "THB", "IDR", "MYR", "PHP", "TWD",
}

# This pass: do not repeat L4i pairs; ban USDSEK; leave CADCHF alone
FORBID = {
    ("NZDUSD", "BUY"),
    ("EURGBP", "BUY"),
    ("EURCNH", "BUY"),
...
def is_exotic(symbol: str) -> bool:
    s = symbol.upper()
    if len(s) >= 6:
        return s[:3] in EXOTIC_CCY or s[3:6] in EXOTIC_CCY
    return any(c in s for c in EXOTIC_CCY)


def base_sl(symbol: str) -> int:
    return BASE_SL_EXOTIC if is_exotic(symbol) else BASE_SL_MAJOR
...
def pick_five() -> list[dict]:
        ...
        cands.append(
            {
                ...
                "sl_tier": "exotic" if is_exotic(sym) else "major",
                "prio": prio,
            }
        )
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks10.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
        ...
        out.append(
            {
                ...
                "config_hash": "level4-demo-sl-major80-exotic250",
            }
        )
    return out
...
def fill_from_history(symbol: str, direction: str, signal_id: str, pts: int, sl: float, result) -> dict | None:
    """Recover fill if position already closed (e.g. instant SL). No new order."""
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4j" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
    ...
    if not pos.sl and sl:
        # one SLTP attempt only if broker dropped SL on fill
...
def finalize(signals, fills, results, cycle_id):
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch10 Level4 L4j; MCP blocked; Python MT5; "
                    "SL majors max(80,stops+spread) / exotics max(250,stops+spread)."
                ),
                "locked_reason_unchanged": True,
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch10 cycle {cycle_id}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch10_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch10 (cycle {cycle_id}) comment L4j",
...
def main() -> int:
    ...
    (EXP / "batch10_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    def _sendable(s):
        rel = " ".join(s.get("relations") or [])
        snap = s.get("raw_snapshot") or {}
        cmp = snap.get("cmp") or ""
        act = ""
        if "Official act " in rel:
            act = rel.split("Official act ", 1)[1].split(" ", 1)[0]
        per = {"M5": 300, "M15": 900, "M30": 1800, "H1": 3600, "H4": 14400}
        tf = s.get("anchor_tf")
        spr = int(s.get("spread_points") or 0)
        age = int(s.get("data_age_seconds") or 0)
        why = ""
        if act not in ("FIRE_BUY", "FIRE_SELL") or "official_act=WAIT_" in cmp:
            why = "not_a_fresh_fire"
        elif tf not in per:
            why = "m1_or_unknown"
        elif spr <= 0 or spr > 20:
            why = "spread"
        elif age >= (2 * per[tf] - 120):
            why = "bar_dying"
        else:
            ...
            if hi is None or lo is None:
                ...
                pt = float(s.get("point_size") or 0.00001)
                edge = (float(snap.get("live_ask")) - float(lo)) if s["research_direction"] == "BUY" else (float(hi) - float(snap.get("live_bid")))
                px = float(snap.get("live_ask") if s["research_direction"] == "BUY" else snap.get("live_bid"))
                if px < float(lo) or px > float(hi):
                    why = "through_bar"
                elif edge <= spr * pt:
                    why = "spread_wider_than_room"
        # #region agent log
        try:
            import json as _json
            from pathlib import Path as _Path
            with _Path(r"C:\Users\C2K\Desktop\MT5 to agent\debug-654892.log").open("a", encoding="utf-8") as _fh:
                _fh.write(_json.dumps({"sessionId": "654892", "hypothesisId": "H1", "location": "run_batch10_l4j.py:_sendable", "message": "gate", "data": {"symbol": s.get("symbol"), "act": act, "why": why or "pass", "age": age, "spr": spr}, "timestamp": int(datetime.now(timezone.utc).timestamp() * 1000)}) + "\n")
        except Exception:
            pass
        # #endregion
        if why:
            log({"event": "reject", "signal_id": s["signal_id"], "symbol": s["symbol"], "text": why})
            return False
        return True

    signals = [s for s in signals if _sendable(s)]
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch10_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4j" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L4j_left": ours_left})
