"""Reconcile L5u cycle 20260922T150704Z from MCP history."""
...
ROWS = [
    dict(
        signal_id="20260922T150704Z_EURGBP_1_S1_1",
        symbol="EURGBP",
        direction="SELL",
        ticket=58575083304,
        entry=0.85764,
    ...
    dict(
        signal_id="20260922T150704Z_USDCAD_1_S3_2",
        symbol="USDCAD",
        direction="BUY",
        ticket=58575083634,
        entry=1.40681,
    ...
    dict(
        signal_id="20260922T150704Z_USDCHF_1_S1_3",
        symbol="USDCHF",
        direction="BUY",
        ticket=58575083903,
        entry=0.82101,
    ...
    dict(
        signal_id="20260922T150704Z_EURAUD_2_S2_4",
        symbol="EURAUD",
        direction="SELL",
        ticket=58575084130,
        entry=1.61053,
    ...
    dict(
        signal_id="20260922T150704Z_EURCHF_1_S1_5",
        symbol="EURCHF",
        direction="BUY",
        ticket=58575084361,
        entry=0.93927,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch43 L5u forced five; soft-avoided L5t; mixed 3 BUY/2 SELL; "
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch43 cycle 20260922T150704Z L5u forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits + " | mix 3 BUY / 2 SELL; avoided L5t and GBPUSD/NZDUSD/AUDUSD SELL stack.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T150704Z", "event": "batch43_complete", "results": results},
    )
    append(LOG, {"event": "batch43_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch43_results.json").write_text(
        json.dumps({"cycle_id": "20260922T150704Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T150704Z" not in md:
        md += (
            "\n## Review 20260922T151300Z — Level 4 batch43 L5u forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 3 BUY / 2 SELL; soft-avoided L5t; no GBPUSD/NZDUSD/AUDUSD "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5u book: **flat**. Foreign Client CADCHF not touched.",
        "",
        "## Batch43 (cycle 20260922T150704Z) comment L5u",
        "Forced five with direction mix (3 BUY / 2 SELL). Soft-avoided L5t. MCP blocked. Python MT5.",
        "Expert timed close ~+5m (18:12:05–07 broker).",
        "",
