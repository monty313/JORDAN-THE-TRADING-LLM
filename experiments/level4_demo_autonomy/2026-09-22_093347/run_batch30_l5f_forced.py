"""Level 4 batch30 L5f FORCED. Magic 771249.
Thin FIRE board must NOT abort. Exactly five quote-backed round-trips.
Soft-avoid L5d pairs; may repeat older pairs to fill five.
...
# Soft prefer: avoid exact L5d pairs when filling five is still possible
SOFT_AVOID = {
    ("GBPUSD", "SELL"),
    ("NZDUSD", "SELL"),
    ("AUDUSD", "SELL"),
...
def pick_five() -> list[dict]:
    ...
    for sym in syms:
        ...
        soft = 1 if (sym, d) in SOFT_AVOID else 0
        stops = int(getattr(info, "trade_stops_level", 0) or 0)
    ...
    (EXP / "_picks30.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
        ...
        out.append(
            {
                ...
                "warning_context": ["Level 4 demo autonomy; L5f forced five"],
                ...
                "config_hash": "level4-demo-forced-five-l5f",
            }
        )
    return out
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L5f" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def main() -> int:
    ...
    picks = pick_five()
    log_plan_vs_board(SCRIPT, [{"symbol": p["symbol"], "side": p["direction"]} for p in picks])
    if len(picks) < 5:
    ...
    (EXP / "batch30_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch30_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
            ...
            "mode": "forced_five_l5f",
            "soft_avoid_l5e": [f"{a} {b}" for a, b in SOFT_AVOID],
        }
    )
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L5f" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L5f_left": ours_left})
    ...
    append_jsonl(
        DECISION,
        {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch30_l5f_held_closed", "n": len(results)},
    )
    mt5.shutdown()
