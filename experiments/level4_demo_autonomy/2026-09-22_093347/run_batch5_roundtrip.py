"""Level 4 batch5 demo round-trip. Magic 771249. Comment L4e. No broker stop."""
...
def topology_for(state: str, reason: str) -> str:
    ...
    if "load" in (state or "").lower() or "pullback" in (reason or ""):
...
def build_signals(picks: list[dict], cycle_id: str, ts_utc: datetime) -> list[dict]:
    ...
    for i, p in enumerate(picks, start=1):
        ...
        ltf = (
            ...
            else ("loaded" if "load" in (p.get("state") or "").lower() or "WAIT_LOADED" in p.get("act", "") else "continuation")
        )
        sig = {
            ...
            "config_hash": "level4-demo-wide-sl-80pts",
        }
        signals.append(sig)
    return signals
...
def open_one(job: dict) -> dict | None:
    ...
    if act not in ("FIRE_BUY", "FIRE_SELL") or "official_act=WAIT_" in cmp or job.get("anchor_tf") == "M1":
        log({"event": "reject", "signal_id": job["signal_id"], "symbol": symbol, "text": "not_a_fresh_fire", "act": act, "anchor_tf": job.get("anchor_tf")})
        return None
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4e" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def close_one(fill: dict) -> dict:
    ...
    if not positions:
        # also search by scanning all positions (ticket filter can race)
        still_open = None
    ...
    exit_px = result.price if result and getattr(result, "price", None) else None
    close_path = "five_minute"
    if not ok or exit_px is None or still is False and not ok:
        hx, hpath = reconcile_exit(ticket)
        if hx is not None:
            exit_px = hx
            close_path = hpath if hpath != "other" else "five_minute"
    ...
    rec = {
        ...
        "close_path": close_path if (ok or not still) else "unknown",
        "flat": not still,
    }
    log(rec)
    return rec
...
def finalize(signals: list[dict], fills: list[dict], results: list[dict], cycle_id: str) -> None:
    ...
    for sig in signals:
        ...
        if exit_px is None:
            ...
            if hx is not None:
                exit_px = hx
                r["exit"] = hx
                r["close_path"] = hpath if hpath != "other" else r.get("close_path")
        ...
        outcome = {
            ...
            "post_outcome_review": "Batch5 Level4 demo; MCP trade blocked; Python MetaTrader5.",
            "locked_reason_unchanged": True,
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Positions re-read after batch5.",
        "",
        f"## Batch5 (cycle {cycle_id})",
    ...
    lines += [
        ...
        "",
        "## Prior batch4 (cycle 20260922T100907Z) scored",
        "NZDUSD/EURUSD/AUDUSD/EURCHF/GBPCHF — all five_minute, flat.",
        "",
        "## Next wake",
...
def main() -> int:
    ...
    (EXP / "batch5_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    ...
    (EXP / "batch5_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
    ...
    # second-pass reconcile any null exits
    for r in results:
        if r.get("exit") is None:
            ...
            if hx is not None:
                ...
                r["exit"] = hx
                r["close_path"] = hpath if hpath != "other" else "five_minute"
                r["net_points"] = round(
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4e" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "positions": left, "our_L4e_left": ours_left})
    (EXP / "batch5_results.json").write_text(
