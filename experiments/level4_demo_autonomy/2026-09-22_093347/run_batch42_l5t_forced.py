"""Level 4 batch42 L5t FORCED. Magic 771249.
...
# Soft prefer: avoid exact L5s pairs when filling five is still possible
SOFT_AVOID = {
    ("AUDNZD", "BUY"),
    ("EURAUD", "SELL"),
    ("USDCAD", "BUY"),
...
def pick_five() -> list[dict]:
    ...
    (EXP / "_picks42.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
        ...
        out.append(
            {
                ...
                "warning_context": ["Level 4 demo autonomy; L5t forced five"],
                ...
                "config_hash": "level4-demo-forced-five-l5t",
            }
        )
    return out
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L5t" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def main() -> int:
    ...
    (EXP / "batch42_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch42_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
            ...
            "mode": "forced_five_l5t",
            "soft_avoid_l5s": [f"{a} {b}" for a, b in SOFT_AVOID],
        }
    )
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L5t" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L5t_left": ours_left})
    ...
    append_jsonl(
        DECISION,
        {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch42_l5t_held_closed", "n": len(results)},
    )
    mt5.shutdown()
