"""Level 4 batch49 L6a FORCED. Magic 771249.
...
# Soft prefer: avoid exact L5z pairs when filling five is still possible
SOFT_AVOID = {
    ("EURUSD", "SELL"),
    ("EURCHF", "BUY"),
    ("EURCAD", "SELL"),
...
def pick_five() -> list[dict]:
    ...
    (EXP / "_picks49.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
        ...
        out.append(
            {
                ...
                "warning_context": ["Level 4 demo autonomy; L6a forced five"],
                ...
                "config_hash": "level4-demo-forced-five-l6a",
            }
        )
    return out
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L6a" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def main() -> int:
    ...
    (EXP / "batch49_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch49_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
            ...
            "mode": "forced_five_l6a",
            "soft_avoid_l5z": [f"{a} {b}" for a, b in SOFT_AVOID],
        }
    )
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L6a" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L6a_left": ours_left})
    ...
    append_jsonl(
        DECISION,
        {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch49_l6a_held_closed", "n": len(results)},
    )
    mt5.shutdown()
