"""Level 4 batch9 L4i. Magic 771249. SL=max(80, stops+spread+2). Hold 5m in-process."""
...
# Prior batch pairs (batch6 L4f, batch7 L4g, batch8 L4h) — do not repeat
FORBID = {
    ("EURCAD", "BUY"),
    ("GBPUSD", "SELL"),
    ("USDCAD", "BUY"),
...
SKIP_SYMBOLS = {"CADCHF"}  # foreign Client open; do not touch symbol
INDEX_LIKE = {
...
def pick_five() -> list[dict]:
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks9.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4i" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def close_one(fill: dict) -> dict:
    ...
    if not ok:
        ...
        # one close attempt only — no retry on reject
...
def finalize(signals, fills, results, cycle_id):
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": "Batch9 Level4 L4i; MCP blocked; Python MT5; SL=max(80,stops+spread+2).",
                "locked_reason_unchanged": True,
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch9 cycle {cycle_id}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch9_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch9 (cycle {cycle_id}) comment L4i",
...
def main() -> int:
    ...
    if ours:
        log({"event": "abort_existing_771249", "positions": [{"ticket": p.ticket, "symbol": p.symbol, "comment": p.comment} for p in ours]})
        mt5.shutdown()
        return 3

    picks = pick_five()
    if len(picks) < 5:
        log({"event": "abort_not_enough_signals", "n": len(picks), "picks": [{"s": p["symbol"], "d": p["direction"]} for p in picks]})
        mt5.shutdown()
        return 4
    ...
    (EXP / "batch9_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch9_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log({"event": "signals_locked", "ts_utc": utc_now(), "cycle_id": cycle_id, "symbols": [j["symbol"] + " " + j["direction"] for j in jobs]})
    ...
    for r in results:
        if r.get("exit") is None:
            ...
            if hx is not None:
                ...
                r["net_points"] = round((hx - r["entry"]) / point if r["direction"] == "BUY" else (r["entry"] - hx) / point, 1)
                r["flat"] = True
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4i" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L4i_left": ours_left})
