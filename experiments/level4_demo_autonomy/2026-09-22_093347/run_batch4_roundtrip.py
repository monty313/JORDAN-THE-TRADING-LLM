"""Level 4 batch4 demo round-trip. Magic 771249. Comment L4d. No broker stop."""
...
def topology_for(state: str, reason: str) -> str:
    if "launch" in (state or "").lower() or "momentum" in (state or "").lower():
        return "launch_continuation_bet"
    if "load" in (state or "").lower():
...
def build_signals(picks: list[dict], cycle_id: str, ts_utc: datetime) -> list[dict]:
    ...
    for i, p in enumerate(picks, start=1):
        ...
        sl = 0.0
        try:
            bar_dt = datetime.strptime(p["closed_bar_time"], "%Y.%m.%d %H:%M:%S").replace(tzinfo=timezone(timedelta(hours=3)))
            age = int((ts_utc - bar_dt.astimezone(timezone.utc)).total_seconds())
        except Exception:
            age = 0
        topo = topology_for(p.get("state") or "", p.get("reason") or "")
        ltf = "launch" if "momentum" in (p.get("state") or "").lower() or "full_body" in (p.get("reason") or "") else (
            "loaded" if "load" in (p.get("state") or "").lower() else "continuation"
        )
        sig = {
            ...
            "reason_locked_before_outcome": (
                ...
                f"Research {direction} for a five-minute demo round-trip with an 80-point protective stop."
            ),
            "known_counterevidence": ["80-point stop is research harness, not doctrine invalidation"],
...
def open_one(job: dict) -> dict | None:
    ...
    sl = 0.0
    if int(job.get("spread_points") or 0) <= 0:
        log({"event": "reject", "signal_id": job["signal_id"], "symbol": symbol, "text": "spread_points_zero"})
        return None
    hi = job.get("high")
    lo = job.get("low")
    if hi is not None and lo is not None:
        if direction == "BUY" and price < float(lo):
            log({"event": "reject", "signal_id": job["signal_id"], "symbol": symbol, "text": "price_under_signal_bar_low", "price": price, "low": lo})
            return None
        if direction == "SELL" and price > float(hi):
            log({"event": "reject", "signal_id": job["signal_id"], "symbol": symbol, "text": "price_over_signal_bar_high", "price": price, "high": hi})
            return None
    period = {"M1": 60, "M5": 300, "M15": 900, "M30": 1800, "H1": 3600}.get(job.get("anchor_tf") or "")
    ...
    if period and cbt:
        ...
        if age >= period * 2:
            log({"event": "reject", "signal_id": job["signal_id"], "symbol": symbol, "text": "stale_closed_bar", "age": int(age), "anchor_tf": job.get("anchor_tf")})
            return None
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4d" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def reconcile_exit(ticket: int, entry: float, direction: str, point: float) -> tuple[float | None, str]:
    from_ts = int(time.time()) - 1800
...
def close_one(fill: dict) -> dict:
    ...
    if not positions:
        exit_px, close_path = reconcile_exit(ticket, entry, direction, point)
        net = None
    ...
    if not ok and not still:
        exit_px, _ = reconcile_exit(ticket, entry, direction, point)
        exit_px = exit_px or price
    net = (exit_px - entry) / point if direction == "BUY" else (entry - exit_px) / point
    rec = {
        "event": "close",
        "ts_utc": utc_now(),
...
def finalize(signals: list[dict], fills: list[dict], results: list[dict], cycle_id: str) -> None:
    fill_by_sid = {f["signal_id"]: f for f in fills}
    res_by_sid = {r["signal_id"]: r for r in results}
    review_bits = []
    for sig in signals:
        sid = sig["signal_id"]
        r = res_by_sid.get(sid)
        f = fill_by_sid.get(sid)
        if r is None:
            append_jsonl(
                FORCED,
                {
        ...
        outcome = {
            ...
            "net_price_after_spread": (exit_px - r["entry"]) if direction == "BUY" else (r["entry"] - exit_px) if exit_px is not None else None,
            "net_points_after_spread": net,
            "outcome_status": status,
            "commission_status": "NOT_INCLUDED",
            "outcome_reason": (
                f"Demo ticket {r['ticket']}: entry {r['entry']}, SL {r['sl']} (80 pts), "
                f"exit {exit_px} via {r['close_path']}. Net {net} points."
            ),
            "post_outcome_review": "Batch4 Level4 demo; MCP trade blocked; Python MetaTrader5.",
            "locked_reason_unchanged": True,
        ...
        review_bits.append(f"{r['symbol']} {direction} {r['close_path']} net={net}")
    ...
    atlas = {
        ...
        "hypotheses": "H-20260922-001/002/003 remain PROPOSED; H-002 not activated. Cycle 20260922T090923Z UNRESOLVED.",
    }
    append_jsonl(ATLAS, atlas)
    append_jsonl(
        DECISION,
        {
            "ts_utc": utc_now(),
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Positions re-read after batch4.",
        "",
        f"## Batch4 (cycle {cycle_id})",
        "MCP trade blocked. Python MetaTrader5. Magic 771249, volume 0.01, SL 80 points.",
        "",
    ...
    for r in results:
        lines.append(
            f"| {r['symbol']} | {r['direction']} | {r['ticket']} | {r['entry']} | {r['sl']} | {r['exit']} | {r['net_points']} | {r['close_path']} | {'yes' if r.get('flat') else 'no'} |"
        )
    for sig in signals:
    ...
    lines += [
        "",
        "No foreign tickets touched. Book should be flat for L4d tickets.",
        "",
        "## Next wake",
...
def main() -> int:
    ...
    # re-read positions before any order
    ...
    (EXP / "batch4_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")

    # Append signal records BEFORE orders
    for sig in signals:
    ...
    jobs = [
        {
            ...
            "symbol": s["symbol"],
            "direction": s["research_direction"],
            "high": (s.get("raw_snapshot") or {}).get("high"),
            "low": (s.get("raw_snapshot") or {}).get("low"),
            "spread_points": s.get("spread_points"),
            "closed_bar_time": s.get("anchor_closed_bar_time"),
            "anchor_tf": s.get("anchor_tf"),
        }
        for s in signals
    ]
    (EXP / "batch4_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log({"event": "signals_locked", "ts_utc": utc_now(), "cycle_id": cycle_id, "n": len(signals), "symbols": [j["symbol"] for j in jobs]})

    # re-read again immediately before orders
    open_pos = list(mt5.positions_get() or [])
    ours = [p for p in open_pos if p.magic == MAGIC]
    if ours:
        log({"event": "abort_pre_open_771249", "ts_utc": utc_now(), "positions": [{"ticket": p.ticket, "symbol": p.symbol} for p in ours]})
        mt5.shutdown()
        return 4
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4d" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "positions": left, "our_L4d_left": ours_left})
    (EXP / "batch4_results.json").write_text(
