"""Level 4 batch11 L4k. Magic 771249.
SL: majors max(80, stops+spread); exotics (JPY/CNH/SEK/…) max(250, stops+spread).
Hold 5m in-process. One attempt. No retry. USDSEK allowed with exotic stop.
"""
...
# This pass: do not repeat L4j pairs; USDSEK allowed (exotic 250); leave CADCHF alone
FORBID = {
    ("USDCAD", "SELL"),
    ("USDCHF", "SELL"),
    ("GBPCHF", "SELL"),
...
def pick_five() -> list[dict]:
    ...
    for c in cands:
        ...
        if len(picked) >= 5:
            break
    (EXP / "_picks11.json").write_text(json.dumps(picked, indent=2), encoding="utf-8")
    return picked
...
def open_one(job: dict) -> dict | None:
    ...
    for p in mt5.positions_get(symbol=symbol) or []:
        if p.ticket not in before and p.magic == MAGIC and "L4k" in (p.comment or ""):
            pos = p
            break
    if pos is None:
        for p in mt5.positions_get(symbol=symbol) or []:
...
def finalize(signals, fills, results, cycle_id):
    ...
    for sig in signals:
        ...
        append_jsonl(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch11 Level4 L4k; MCP blocked; Python MT5; "
    ...
    append_jsonl(
        ...
        {
            "ts_utc": utc_now(),
            "review": f"Level4 batch11 cycle {cycle_id}",
    ...
    append_jsonl(DECISION, {"ts_utc": utc_now(), "cycle_id": cycle_id, "event": "batch11_complete", "results": results})
    ...
    lines = [
        ...
        "",
        f"## Batch11 (cycle {cycle_id}) comment L4k",
        "MCP blocked. Python MT5. Magic 771249, 0.01.",
        "SL: majors max(80, stops+spread); exotics max(250, stops+spread). USDSEK allowed at exotic 250.",
        "",
...
def main() -> int:
    ...
    (EXP / "batch11_signals.json").write_text(json.dumps(signals, indent=2), encoding="utf-8")
    for sig in signals:
        append_jsonl(FORCED, sig)
    jobs = [{"signal_id": s["signal_id"], "symbol": s["symbol"], "direction": s["research_direction"]} for s in signals]
    (EXP / "batch11_jobs.json").write_text(json.dumps(jobs, indent=2), encoding="utf-8")
    log(
        {
    ...
    ours_left = [p for p in left if p["magic"] == MAGIC and "L4k" in (p.get("comment") or "")]
    log({"event": "batch_done", "ts_utc": utc_now(), "left": left, "our_L4k_left": ours_left})
    (EXP / "batch11_results.json").write_text(
