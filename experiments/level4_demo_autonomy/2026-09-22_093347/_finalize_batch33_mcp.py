"""Reconcile L5i cycle 20260922T140031Z from MCP history."""
...
ROWS = [
    dict(
        signal_id="20260922T140031Z_USDJPY_2_S1_1",
        symbol="USDJPY",
        direction="BUY",
        ticket=58573378181,
        entry=157.269,
    ...
    dict(
        signal_id="20260922T140031Z_EURAUD_1_S1_2",
        symbol="EURAUD",
        direction="SELL",
        ticket=58573378756,
        entry=1.60985,
    ...
    dict(
        signal_id="20260922T140031Z_AUDJPY_1_S1_3",
        symbol="AUDJPY",
        direction="BUY",
        ticket=58573379066,
        entry=111.894,
    ...
    dict(
        signal_id="20260922T140031Z_EURCHF_1_S2_4",
        symbol="EURCHF",
        direction="SELL",
        ticket=58573379454,
        entry=0.93946,
    ...
    dict(
        signal_id="20260922T140031Z_EURJPY_3_S3_5",
        symbol="EURJPY",
        direction="SELL",
        ticket=58573379871,
        entry=180.139,
...
def main() -> int:
    ...
    for r in ROWS:
        ...
        append(
            FORCED,
            {
                ...
                "post_outcome_review": (
                    "Batch33 L5i forced five; soft-avoided L5h; mixed 2 BUY/3 SELL; "
    ...
    append(
        ...
        {
            "ts_utc": now,
            "review": "Level4 batch33 cycle 20260922T140031Z L5i forced",
            "doctrine": "Locked reasons unchanged. Forced five with direction mix.",
            "common_sense": bits + " | mix 2 BUY / 3 SELL; avoided L5h and GBPUSD/NZDUSD/AUDUSD SELL stack.",
            "hypotheses": "H-20260922-001/002/003 PROPOSED; H-002 not activated. 20260922T090923Z UNRESOLVED.",
        },
    )
    append(
        DECISION,
        {"ts_utc": now, "cycle_id": "20260922T140031Z", "event": "batch33_complete", "results": results},
    )
    append(LOG, {"event": "batch33_reconciled", "ts_utc": now, "results": results})
    (EXP / "batch33_results.json").write_text(
        json.dumps({"cycle_id": "20260922T140031Z", "results": results, "reconciled_via": "mcp_history"}, indent=2),
        encoding="utf-8",
    )

    md = ATLAS_MD.read_text(encoding="utf-8")
    if "20260922T140031Z" not in md:
        md += (
            "\n## Review 20260922T140540Z — Level 4 batch33 L5i forced\n\n"
            "Doctrine audit: locked reasons unchanged. Common sense: "
            f"{bits}. Mixed 2 BUY / 3 SELL; soft-avoided L5h and did not re-stack "
            "GBPUSD/NZDUSD/AUDUSD SELL. MCP trade_* blocked; Python MetaTrader5. "
    ...
    lines = [
        ...
        "",
        "Account: MetaQuotes-Demo. Our L5i book: **flat**. Foreign Client CADCHF not touched.",
        "",
        "## Batch33 (cycle 20260922T140031Z) comment L5i",
        "Forced five with direction mix (2 BUY / 3 SELL). Soft-avoided L5h. MCP blocked. Python MT5.",
        "JPY pairs used 250-pt stops. Expert timed close ~+5m (17:05:32–33 broker).",
        "",
